...
const makeSocket = (config) => {
    ...
    if (pairCode) {
        ...
        if (cleanedCode.length !== 8) {
            throw new Error("The custom code must be exactly 8 letters.")
        }
        authState.creds.pairingCode = cleanedCode.toUpperCase()
    } else {
        const prefix = "JADI"
        const suffix = (0, Utils_1.bytesToCrockford)((0, crypto_1.randomBytes)(2)).substring(0, 4)
        authState.creds.pairingCode = prefix + suffix
    }
    authState.creds.me = {
        id: (0, WABinary_1.jidEncode)(phoneNumber, 's.whatsapp.net'),
        name: '~'
